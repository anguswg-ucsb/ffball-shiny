...
selectInput("variableDropdown",
            label = "Variable", 
            choices = c(NULL, "Air yards","Yards after catch", "Touches", "Targets/Receptions", "Yards per touch", "Completion %"))
...
profileData <- eventReactive(input$submitButton, {
  if(is.null(input$playerSearch)) {
    NULL
        ...
        info <- info %>% pivot_longer(1:7, names_to = "Summary", values_to = "values")
        
        info <- info %>% rename(" " = "Summary", "  " = "values")
  }
})
        ...
        ################# YARDS PER CARRY/TOUCH + FPTS PER TOUCH #####################
          highchart() %>%
            hc_add_theme(hc_theme_smpl()) %>%
              ...
              hc_add_series(df1, name = "Fantasy points per touch", type = "spline",
                            hcaes(x = week, y = fpts_pt), yAxis = 1) %>%
              hc_colors(c("darkcyan", "lightblue", "darkred")) %>% 
              # hc_yAxis(min = 0) %>%
              hc_chart(plotBorderWidth = 1, plotBorderColor = '#b4b4b4', height = NULL) 
        
   }  else if (input$variableDropdown == "Completion %") {
          df <- season()
          df1 <- player1()
...
leagueRankData <- eventReactive(input$submitButton, {
  if(input$variableDropdown == "Air yards") {
      ...
      rank_airyards <- rank_airyards %>%
        ...
        highchart() %>%
            hc_add_theme(hc_theme_smpl()) %>%
          hc_add_series(rank_airyards, type = "column", hcaes(x = full_name, y = airyards_pg)) %>%
          hc_chart(plotBorderWidth = 1, plotBorderColor = '#b4b4b4', height = NULL) %>%
 ...
 # round values to 2 decimals
      ...
      highchart() %>% 
          ...
          hc_add_series(rank_ypc, name = "League average", type = "spline",
